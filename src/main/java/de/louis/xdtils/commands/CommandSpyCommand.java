...
public class CommandSpyCommand implements CommandExecutor {
    ...
    public CommandSpyCommand(SpyManager spyManager) {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!player.hasPermission("xdtils.commandspy")) {
            player.sendMessage(MessageUtil.noPermission(label));
            return true;
        }

        boolean now = spyManager.toggleCommandSpy(player.getUniqueId());
        player.sendMessage(MessageUtil.prefixed("<gray>CommandSpy "
